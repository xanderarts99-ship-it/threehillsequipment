...
export default function EditMachineDrawer({ isOpen, machine, onClose }: EditMachineDrawerProps) {
    ...
    if (isOpen) {
        if (machine) {
            setFormData({
                ...
                gallery: machine.gallery || [] // Initialize gallery
            });
            // Initialize gallery previews with existing URLs and mark them as such
  ...
  const handleGalleryChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
        ...
        // Take only as many files as we have room for
  ...
  const removeGalleryImage = async (index: number) => {
    ...
    if (galleryPreviews[index].startsWith('blob:')) {
        const fileIndex = galleryFiles.findIndex((_, i) => URL.createObjectURL(galleryFiles[i]) === galleryPreviews[index] || i === (index - (formData.gallery?.length || 0)));
        // Note: The above logic is simplified. Correctly strictly separating existing vs new is safer.
        
        // Let's rely on index relative to arrays
        const existingCount = formData.gallery?.length || 0;
        
        if (index < existingCount) {
             // Removing valid existing image from DB array
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-2xl bg-[#111] border-l border-white/10 h-full overflow-y-auto p-8 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Main Image Upload */}
            <div>
                <label className="block text-xs font-bold uppercase text-white/40 mb-2">Main Image (Thumbnail)</label>
                <div className="flex items-center gap-4">
                    {formData.image && (
                         <div className="w-20 h-20 bg-white/5 border border-white/10 relative">
                    ...
                    {imageFile && <span className="text-xs text-white/60">{imageFile.name}</span>}
                </div>
            </div>
            ...
            <div>
                ...
                <p className="text-xs text-white/30 italic">Upload up to {MAX_GALLERY_IMAGES} images for the detail page gallery. ({galleryPreviews.length}/{MAX_GALLERY_IMAGES})</p>
            </div>
            ...
            <div>
                <div className="flex items-center justify-between mb-4 mt-8 pt-8 border-t border-white/10">
                    ...
                    <button 
                        type="button"
                        onClick={addSpec}
                        className="flex items-center gap-1 text-[var(--gold)] text-xs font-bold hover:underline"
                    >
                ...
                <div className="space-y-2">
                    {formData.specs?.map((spec, i) => (
                        <div key={i} className="flex gap-4">
                            ...
                            <input 
                                className="flex-1 bg-white/5 border border-white/10 p-2 text-sm text-white focus:border-[var(--gold)] outline-none"
                                placeholder="Value (e.g. V8 Turbo)"
                                value={spec.value}
                    ...
                    {formData.specs?.length === 0 && (
                        <p className="text-white/20 text-sm italic">No specifications added.</p>
                    )}
                </div>
            </div>
